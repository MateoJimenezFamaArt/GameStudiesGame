...
private void DetermineWinner()
{
    ...
    // Highlight the winning combination in the UI
    private void HighlightWinningCombination(List<Dice> diceSet, bool isWinner)
    {
        Color highlightColor = Color.yellow; // Color for the winning combination

        for (int i = 0; i < diceSet.Count; i++)
        {
            // Determine the index in the UI TextMeshPro array
            int uiIndex = diceSet == playerDice ? i + bankDice.Count : i; // Adjust index based on player or bank dice
            diceResultTexts[uiIndex].color = isWinner ? highlightColor : Color.black; // Change to highlight color or reset to black
        }
    }
